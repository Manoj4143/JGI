...
// USER RULE: For teacher, any theory class MUST have at least 1 free period before and after!
function isTeacherTheoryIsolated($teacher, $day, $slot, &$teacher_busy) {
    if (isset($teacher_busy[$teacher][$day][$slot])) return false;
    if ($slot > 1 && isset($teacher_busy[$teacher][$day][$slot - 1])) return false;
    if ($slot < 7 && isset($teacher_busy[$teacher][$day][$slot + 1])) return false;
    return true;
}
...
function markSlotBusy($teacher, $room, $group, $day, $slot, $sub_id, &$teacher_busy, &$room_busy, &$group_busy, &$group_day_subs) {
        ...
        foreach ($rotated_subs as $sub) {
            ...
            foreach ($cand_days as $d) {
                ...
                foreach ($slot_order as $slot) {
                    ...
                    // Teacher must have 1-period free before and after!
                    if (!isTeacherTheoryIsolated($tid, $d, $slot, $teacher_busy)) continue;
                    ...
                    foreach ($lecRooms as $lr) {
                        ...
                        if (isSlotAvailable($tid, $rid, $grp, $d, $slot, $teacher_busy, $room_busy, $group_busy)) {
                            markSlotBusy($tid, $rid, $grp, $d, $slot, $sid, $teacher_busy, $room_busy, $group_busy, $group_day_subs);
                            $schedule_entries[] = [
                                'room_id' => $rid,
                                'sub_id' => $sid,
                                'teacher_id' => $tid,
...
foreach ($group_theory_subs as $grp => $sub_list) {
    foreach ($sub_list as $sub) {
        ...
        while ($allocated_hours[$sid] < $needed) {
            ...
            if (!$placed) {
                // Fallback: at least 1-period buffer from same subject, but teacher is still strictly isolated!
                foreach ($days as $d) {
                    if (isset($group_day_subs[$grp][$d][$sid]) && $group_day_subs[$grp][$d][$sid] >= 2) continue;
                    foreach ([5, 6, 1, 2, 3, 4] as $slot) {
                        if (isset($group_busy[$grp][$d][$slot])) continue;
...
// 1. isTeacherTheoryIsolated (1 free period before and after theory class)
                    ...
                    for ($later = $max_s; $later > $s; $later--) {
                        if (isset($g_slots[$later])) {
                            ...
                            if (in_array($entry['subject_type'], ['THEORY', 'ONE_CREDIT'])) {
                                ...
                                $placed_new = false;
                                if (isTeacherTheoryIsolated($tid, $d, $s, $teacher_busy) && 
                                    ...
                                    foreach ($lecRooms as $lr) {
                                        ...
                                        if (!isset($room_busy[$nrid][$d][$s])) {
                                            markSlotBusy($tid, $nrid, $grp, $d, $s, $sid, $teacher_busy, $room_busy, $group_busy, $group_day_subs);
