...
for ($test_seed = 0; $test_seed <= 50; $test_seed++) {
    ...
    for ($round = 0; $round < 6; $round++) {
        foreach ($group_theory_subs as $grp => $sub_list) {
            ...
            $offset = ($round + $test_seed) % count($rotated_subs);
                    ...
                    foreach ($slots as $slot) {
                        ...
                        foreach ($lecRooms as $lr) {
                            ...
                            if (isSlotAvailable($tid, $rid, $grp, $d, $slot, $teacher_busy, $room_busy, $group_busy)) {
                                $best_d = $d; $best_s = $slot; $best_room = $rid;
                                break 2;
                            }
                        }
                    }
                    if ($best_d !== null) break;
                }
                if ($best_d !== null) {
                    markSlotBusy($tid, $best_room, $grp, $best_d, $best_s, $sid, $teacher_busy, $room_busy, $group_busy, $group_day_subs, true, $teacher_theory);
                    $allocated_hours[$sid]++;
                    $schedule_entries[] = 1;
                }
            }
    ...
    $cnt = count($schedule_entries);
    if ($cnt >= 142) {
        echo "Seed $test_seed => Total scheduled: $cnt\n";
    }
}
